  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="mb-3">
          <input v-model="$store.state.freeboard.title" type="text" class="form-control" placeholder="제목">
        </div>
        <div class="mb-3">
        ...
        <div v-if="$store.state.freeboard.fileList.length > 0" class="mb-3">
          첨부파일 : <span><div v-for="(file, index) in $store.state.freeboard.fileList" class="fileName" :key="index">{{file.fileName}}</div></span>
        </div>
  ...
  data() {
    ...
    methods: {
        ...
        boardUpdate(){
          ...
          formData.append("boardId", this.$store.state.freeboard.boardId); // update 에 추가
          formData.append("title", this.$store.state.freeboard.title);
          ...
          http.post(
            '/freeboards/' + this.$store.state.freeboard.boardId, // store boardId 
            formData,
